...
export default function Navbar() {
  ...
  useEffect(() => {
    const onScroll = () => {
      const h = document.querySelector('.hero-section')?.offsetHeight ?? 0;
            ...
            <div className="hidden md:flex items-center gap-3">
              {user ? (
                <>
                  ...
                  {user.role === 'admin' && (
                    <a href="/admin" className="text-xs font-semibold px-3 py-1.5 rounded-lg bg-brand-100 text-brand-700 hover:bg-brand-200 transition">Admin</a>
                  )}
