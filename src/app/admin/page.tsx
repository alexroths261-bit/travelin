...
import { BarChart3, Package, Users, IndianRupee, ArrowLeft, Clock, CheckCircle2, XCircle } from "lucide-react";
...
export default function AdminPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/bookings").then(r => {
      ...
      if (data) { setBookings(data.bookings); setStats(data.stats); }
    ...
    <div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
          {[
            { label: "Total Revenue", value: stats.revenue, icon: IndianRupee, color: "bg-brand-100 text-brand-700" },
            { label: "Total Bookings", value: stats.bookings, icon: BarChart3, color: "bg-blue-100 text-blue-700" },
            { label: "Total Users", value: stats.users, icon: Users, color: "bg-purple-100 text-purple-700" },
            { label: "Active Packages", value: stats.packages, icon: Package, color: "bg-amber-100 text-amber-700" },
          ].map(s => (
            <div key={s.label} className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm text-gray-500 font-medium">{s.label}</span>
                <div className={`w-9 h-9 rounded-xl ${s.color} flex items-center justify-center`}><s.icon className="w-4.5 h-4.5" /></div>
              </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          {bookings.length === 0 ? (
            <div className="px-6 py-16 text-center"><Package className="w-10 h-10 text-gray-300 mx-auto mb-3" /><p className="text-gray-400 text-sm">No bookings yet. They will appear here once users start booking.</p></div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="bg-gray-50 text-left"><th className="px-6 py-3 text-gray-500 font-semibold text-xs uppercase tracking-wider">User</th><th className="px-6 py-3 text-gray-500 font-semibold text-xs uppercase tracking-wider">Package</th><th className="px-6 py-3 text-gray-500 font-semibold text-xs uppercase tracking-wider">Travelers</th><th className="px-6 py-3 text-gray-500 font-semibold text-xs uppercase tracking-wider">Amount</th><th className="px-6 py-3 text-gray-500 font-semibold text-xs uppercase tracking-wider">Status</th><th className="px-6 py-3 text-gray-500 font-semibold text-xs uppercase tracking-wider">Date</th></tr></thead>
                <tbody className="divide-y divide-gray-50">
                  {bookings.map(b => (
                    <tr key={b.id} className="hover:bg-gray-50/50 transition">
                      ...
                      <td className="px-6 py-4 text-gray-400">{new Date(b.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
